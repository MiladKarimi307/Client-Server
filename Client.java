...
public class Client {
			...
			if (isValid(comm)) {
				...
				if(comm.equals("8")) {
					...
					if (isValid(s)) {
						...
						for(int i = 0; i < n; i++) {
							cList.get(i).run();
							System.out.println(" "+ (i+1) +")   Latency: "+ (cList.get(i).timeB-cList.get(i).timeA) +"ms");
							average += cList.get(i).timeB-cList.get(i).timeA;
						}
						System.out.println(" The average latency is: "+ average/n +"ms and the whole proccess took "+ average +"ms/"+ average/1000 +"s");
						try {
							while(socketInputStream.available() > 0) socketInputStream.readUTF();
						} catch (IOException e) {
							e.printStackTrace();
						}
					}
	...
	public static class thread extends Client implements Runnable {
		...
		String request;
		String reply;
		public thread(String hostAddress, Integer port, String request)	throws UnknownHostException, IOException, NumberFormatException {
